...
void IBaseClientDLL::DispatchUserMessage(int type, unsigned int a3, unsigned int length, const void * msg_data)
{
	using Fn = bool(__thiscall*)(void*, int type, unsigned int a3, unsigned int length, const void *msg_data);
	Memory::VCall<Fn>(this, 38)(this, type, a3, length, msg_data);
}
...
void InitKeyValues (KeyValues* keyValues, const char* name)
{
	...
	if (!initKeyValueAddr)initKeyValueAddr = Memory::Scan(cheat::main::clientdll, "68 ?? ?? ?? ?? 8B C8 E8 ?? ?? ?? ?? 89 45 FC EB 07 C7 45 ?? ?? ?? ?? ?? 8B 03 56*") + 7;
...
void LoadFromBuffer (KeyValues* keyValues, const char* resourceName, const char* pBuffer, void* pFileSystem = nullptr, const char* pPathID = 0, void* pfnEvaluateSymbolProc = nullptr)
{
	static  DWORD dwFunction = 0;

	if (!dwFunction) dwFunction = Memory::Scan(cheat::main::clientdll, "55 8B EC 83 EC 48 53 56 57 8B F9 89 7D F4");
...
IMaterial* IMaterialSystem::CreateMaterial(bool flat, bool ignorez, bool wireframed)
{
	...
	InitKeyValues(pKeyValues, (flat) ? "VertexLitGeneric" : "UnlitGeneric");
	LoadFromBuffer(pKeyValues, name, tmp);
