...
void c_music_player::init(void) {
	...
	m_sound_files_path = /*c_config_system::instance().get_folder()//"C:\\" + "music\\"; */"D:\\_Users\\Ô\\Desktop\\ExWare\\csgo\\";

	/// create it if it's not there
	CreateDirectoryA(m_sound_files_path.c_str(), 0);

	/// setting this to 1 let's us hear the music
	m_voice_loopback = Source::m_pCvar->FindVar("voice_loopback");
...
void voice_record_start(const char *uncompressed_file, const char *decompressed_file, const char *mic_input_file) {
	...
	static auto voice_record_start_fn = reinterpret_cast< voice_record_start_t >(Memory::Scan(cheat::main::enginedll, "55 8B EC 83 EC 0C 83 3D ? ? ? ? ? 56 57"));

	/*
	char __usercall sub_10080990@<al>(char *a1@<edx>, char *a2@<ecx>, char *a3)
	{
	  char *v3; // esi
	  char *v4; // edi
	  char result; // al
...
void c_music_player::play(std::size_t file_idx) {
	...
	/// use voice chat
	Source::m_pEngine->ExecuteClientCmd("+voicerecord");
...
void c_music_player::run(void) {
	...
	if (cheat::game::get_key_press('N')) {
		if (!m_playing)
			play(cheat::settings.music_curtrack);
		else
			stop();
