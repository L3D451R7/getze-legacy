...
public:
	virtual bool RegisterConCommandBase(ConCommandBase *pVar)
	{
		...
		Source::m_pCvar->RegisterConCommand(pVar);
		return true;
	}
};
...
{
	if (!Source::m_pCvar || s_bRegistered)
		return;

	assert(s_nDLLIdentifier < 0);
	s_bRegistered = true;
	s_nCVarFlag = nCVarFlag;
	s_nDLLIdentifier = Source::m_pCvar->AllocateDLLIdentifier();
...
void ConVar_Unregister()
{
	...
	assert(s_nDLLIdentifier >= 0);
	Source::m_pCvar->UnregisterConCommands(s_nDLLIdentifier);
	s_nDLLIdentifier = -1;
	s_bRegistered = false;
}
...
{
	if (Source::m_pCvar) {
		Source::m_pCvar->UnregisterConCommand(this);
	}
}
...
void ConVar::ChangeStringValue(const char *tempVal, float flOldValue)
{
	...
	if (Source::m_pCvar)
		Source::m_pCvar->CallGlobalChangeCallbacks(this, pszOldValue, flOldValue);
}
...
{
	m_pOriginalCVar = Source::m_pCvar->FindVar(szCVar);
	Spoof();
}
...
SpoofedConvar::SpoofedConvar(ConVar* pCVar)
{
	...
	if (!IsSpoofed() && m_pOriginalCVar)
	{
		...
		//Register it
		Source::m_pCvar->RegisterConCommand(m_pDummyCVar);
...
{
	return (this != nullptr && m_pDummyCVar != nullptr);
}
...
void SpoofedConvar::Restore()
{
	if (this && IsSpoofed())
	{
		...
		Source::m_pCvar->UnregisterConCommand(m_pDummyCVar);
