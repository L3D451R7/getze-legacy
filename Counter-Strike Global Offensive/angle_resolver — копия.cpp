...
void c_resolver::resolve(C_BasePlayer* m_player, int &history, int &chocked_ticks)
{
	...
	if ((lag_data->last_simtime - m_player->m_flSimulationTime()) > 0.5f) {
		...
		if (lag_data->origin.Distance(m_player->m_vecOrigin()) > 1.f && fabs(Math::AngleDiff(lag_data->last_lby, m_player->m_flLowerBodyYawTarget())) > 20.f) {
			lag_data->saved_off_delta = lag_data->move_lby = FLT_MAX;
		}
	}

	lag_data->did_shot_this_tick = (m_player->get_weapon() != nullptr && m_player->get_weapon()->m_flLastShotTime() != lag_data->last_shot_time && (m_player->m_angEyeAngles().x != lag_data->previous_angles.x || fabs(m_player->m_angEyeAngles().x) < 65));//((activity >= ACT_CSGO_FIRE_PRIMARY && activity <= ACT_CSGO_FIRE_SECONDARY_OPT_2) && resolve_data->server_anim_layers[1].m_flWeight >= 0.01f && m_player->get_animation_layer(1).m_flCycle < 0.05f) || (m_player->get_weapon() && m_player->get_weapon()->m_Activity() == 208);
	...
	/*if (state->speed_2d > 0.1f) 
	{
		...
		lag_data->next_lby_time = Source::m_pGlobalVars->curtime + 0.22f;
		lag_data->prev_lby_time = Source::m_pGlobalVars->curtime;
	}
	else 
	{
		if (lby != lag_data->prev_lby && lag_data->prev_lby != FLT_MAX) {
			lag_data->did_lby_flick = true;
			lag_data->next_lby_time = Source::m_pGlobalVars->curtime + 1.1f;
			lag_data->prev_lby_time = Source::m_pGlobalVars->curtime;
		}
		else
			lag_data->prev_lby = lby;

		if ((lag_data->next_lby_time - Source::m_pGlobalVars->curtime) < 0.0f && lag_data->next_lby_time < FLT_MAX)
		{
			lag_data->did_lby_flick = true;
			lag_data->next_lby_time = Source::m_pGlobalVars->curtime + 1.1f;
		}
	...
	if ((lag_data->server_anim_layers[3].m_flWeight <= 0.2f && m_player->get_sec_activity(lag_data->server_anim_layers[3].m_nSequence) == 979 && lag_data->server_anim_layers[3].m_flCycle < 1.f)) 
	{
		lag_data->did_lby_flick = true;
		lag_data->next_lby_time = Source::m_pGlobalVars->curtime + 1.1f;
	}*/

	if (lag_data->deltas_count > 2)
		lag_data->avg_delta = float(lag_data->total_deltas / lag_data->deltas_count);
	...
	if(fabs(Math::AngleDiff(lby, lag_data->prev_lby)) > 30.f && lag_data->prev_lby != FLT_MAX) {
		...
		const auto poopoo = Math::normalize_angle(Math::AngleDiff(lby, lag_data->move_lby));
		lag_data->saved_off_delta = poopoo;

		lag_data->deltas_count++;
		lag_data->total_deltas += fabs(poopoo);

		if (lag_data->deltas_count > 5) {
			lag_data->deltas_count = 0;
			lag_data->total_deltas = 0.f;
		}
	...
	if (lag_data->fakeangles && cheat::Cvars.RageBot_Resolver.GetValue() && !m_player->IsBot())
	{
		const auto on_ground = m_player->m_fFlags() & FL_ONGROUND/* && lag_data->prev_flags & FL_ONGROUND*/;

		if (m_player->m_vecVelocity().Length2D() > 0.1f && on_ground)
		{
			lag_data->did_lby_flick = true;
		...
		{
			if (lag_data->did_lby_flick && lag_data->missed_shots[2] < 2 && on_ground)
			{
				m_player->m_flSimulationTime() = m_player->m_flOldSimulationTime();
				m_player->m_angEyeAngles().y = Math::normalize_angle(lby);
			...
			else
			{
				...
				auto lol = cheat::main::shots_fired[idx] % (int)brutelist.size();
